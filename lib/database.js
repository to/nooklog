...
class NookmarkDatabase {
	...
	async search({ tags = [], query = '', fields = [], minRating, sortBy = 'updated_at', limit = 200 }) {
		let builder = this.bookmarks.query();

		const conditions = [];
		for (const tag of tags)
			conditions.push(sql`array_contains(tags, ${tag})`);
		if (minRating != null)
		...
		let results = await builder.limit(10000).toArray();
		if (query) {
