...
> [!INFO]
> 少量のデータを取得したり 検索するときは OpenAPIも利用できます。 稼働しているサーバーの /openapi.htmlを確認してください。
> 編集画面でのURLやタグの補正は Greasemonkeyで パッチできます。 tool/userjs/*.jsを確認してください。

## メンテナンスの基本方針

- 安全のために 起動しているNooklogは 終了しておきます。
...
- **データベーススキーマの確認**: [📖 schema.md](../../../docs/schema.md) を確認し、データベーススキーマの最新の定義を把握しておくと作業がスムーズに進められます。
...
- `bookmark`テーブルの `meta`カラム（JSONオブジェクト）に 自分だけのデータを足すときは 必ず _からはじまる名前にします。
